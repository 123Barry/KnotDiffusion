...
"""
# add work place to system path
import sys
import os
current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.abspath(os.path.join(current_dir, '..', 'KnotDiffusion'))
...
class Sampler:
    ...
    def run_sampling(self):
        ...
        """
        # add: sample 2000 samples of random lengths
        sampled_lengths = random.choices(range(self._sample_conf.min_length, 
                                               self._sample_conf.max_length + 1),k=2000) # 2000 samples
        for sample_i, sample_length in enumerate(sampled_lengths):
                unique_id = f'{sample_length}_{sample_i}'
                length_dir = os.path.join(
                     self._output_dir, f'length_{unique_id}')
                ...
                sc_output_dir = os.path.join(length_dir, 'self_consistency')
